...
func proposeForType(nodeID NodeID, nodeType NodeType) (string, string, []ProposedEvent) {
	...
	case NodeExpression:
		return "式の更新", "計算式が影響を受けるため再検討が必要", []ProposedEvent{
			{Event: Event{Type: EventAttrUpdated, NodeID: nodeID, Attrs: Attrs{"repair_hint": VString("update formula")}}, Note: "式の更新が必要", Applyable: false, AutoLevel: ManualOnly},
		}
	case NodeField:
		return "フィールドの確認", "型/制約/既定値を確認", []ProposedEvent{
			{Event: Event{Type: EventAttrUpdated, NodeID: nodeID, Attrs: Attrs{"repair_hint": VString("review field")}}, Note: "影響を受けるため確認", Applyable: false, AutoLevel: NeedsReview},
		}
	case NodeForm:
		return "フォームの確認", "表示/入力の整合性を確認", []ProposedEvent{
			{Event: Event{Type: EventAttrUpdated, NodeID: nodeID, Attrs: Attrs{"repair_hint": VString("review form")}}, Note: "フォームの整合性確認", Applyable: false, AutoLevel: NeedsReview},
		}
	case NodeList:
		return "一覧の確認", "列定義/表示内容を確認", []ProposedEvent{
			{Event: Event{Type: EventAttrUpdated, NodeID: nodeID, Attrs: Attrs{"repair_hint": VString("review list")}}, Note: "一覧の整合性確認", Applyable: false, AutoLevel: NeedsReview},
		}
	case NodeRole:
		return "権限の確認", "アクセス制御を確認", []ProposedEvent{
			{Event: Event{Type: EventAttrUpdated, NodeID: nodeID, Attrs: Attrs{"repair_hint": VString("review role")}}, Note: "権限の見直し", Applyable: false, AutoLevel: NeedsReview},
		}
	case NodeEntity:
		return "エンティティの確認", "構造/関連の整合性を確認", []ProposedEvent{
			{Event: Event{Type: EventAttrUpdated, NodeID: nodeID, Attrs: Attrs{"repair_hint": VString("review entity")}}, Note: "構造の見直し", Applyable: false, AutoLevel: NeedsReview},
		}
	case NodeRelation:
		return "リレーションの確認", "関係の整合性を確認", []ProposedEvent{
			{Event: Event{Type: EventAttrUpdated, NodeID: nodeID, Attrs: Attrs{"repair_hint": VString("review relation")}}, Note: "関係の見直し", Applyable: false, AutoLevel: NeedsReview},
		}
	case NodeParam:
		return "パラメータの確認", "依存先との整合性を確認", []ProposedEvent{
			{Event: Event{Type: EventAttrUpdated, NodeID: nodeID, Attrs: Attrs{"repair_hint": VString("review param")}}, Note: "パラメータの確認", Applyable: false, AutoLevel: NeedsReview},
		}
	default:
		return "確認", "影響を受けるため関連する設定を確認", []ProposedEvent{
			{Event: Event{Type: EventAttrUpdated, NodeID: nodeID, Attrs: Attrs{"repair_hint": VString("review")}}, Note: "影響の確認", Applyable: false, AutoLevel: NeedsReview},
		}
	}
}
