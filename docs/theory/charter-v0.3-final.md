...
Palimpsest はローコード SaaS の設定変更問題を、ビルドシステムの増分再ビルド問題と**計算構造として本質的に等価**と見なし、確立された増分計算理論を適用する。

「同型（isomorphic）」ではなく「本質的に等価」としたのは、完全な同型ではないからである（§2.3 で差異を論じる）。しかし、増分計算の観点からは同じ手法が適用可能であり、その理論的・実践的蓄積を活用できる。

本憲章では、この等価性がなぜ成り立つのか、どのような形式化が適切か、そしてその形式化がなぜ十分かを論じる。
...
**なぜこの 4 種類か**: MVP として、ローコード SaaS で頻出する依存パターンをカバーする最小セット。`uses` と `derives` はデータフロー、`controls` と `constrains` は制御フローに対応する。

この分類は「影響の伝播」と「検証の責務」を分離するためのものであり、意味論的な完全性よりも、**運用上の安全性と説明可能性を優先した設計**である。理論的に最小なラベル集合ではなく、運用と UX において最小限必要なラベル集合を選んでいる。将来の拡張で追加可能だが、まず最小セットで検証する。
...
将来的には、実行時トレースによる依存集合の精密化（Adapton 等の self-adjusting computation）を導入する余地がある。しかし v0.3 では、実装複雑性と安全性のトレードオフを考慮し、静的近似を採用する。これは「知らない」のではなく「知った上で今はやらない」判断である。
